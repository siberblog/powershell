# ==============================================================================================
#    NAME: VNK_Log_Analysis_v7.9.1.ps1
#  AUTHOR: Bekir Yalcin
#    DATE: 2018/12/04
...
$SearchString22 = "ERR_NOVEHICLE_INSIDE_PROBLEM"
...
$SearchString17 = "GOT RESPONSE FROM TELLER ACCOUNT QUERY"
$SearchString18 = "Received Tag: Tag\(READ,UHF"
...
$ReadRate =  0 #@() 
$ReadRate =  [System.Math]::Round((($RFID.count - $M_RFID.count) / $RFID.count) * 100, 2)
...
# --Sensör delay hatası ERR_NOVEHICLE--
    $ERR_NOVEHICLE = $FilterValues | select-string -pattern $SearchString22 -CaseSensitive 
    if ($SearchString22.Length -gt 0) { Write-Host " " (('Sensor delay - ERR_NOVEHICLE'.Replace('\','')).PadRight($LineLengtgh)).ToString() ": " -NoNewline }
    if ($ERR_NOVEHICLE.Count -gt 0 -and $ERR_NOVEHICLE.Count -lt 10) { Write-Host $ERR_NOVEHICLE.Count -f Yellow } 
    elseif ($ERR_NOVEHICLE.Count -gt 10 ) { Write-Host $ERR_NOVEHICLE.Count -f Red } else { Write-Host $ERR_NOVEHICLE.Count }     
...
if ($SearchString41.Length -gt 0 -or $SearchString39.Length -gt 0 ) { Write-Host " " "Debounce Loops".PadRight($LineLengtgh)  ": " -NoNewline
																	  Write-Host $Debounce"_"$Debounce2 -ForegroundColor Cyan
...
# -------------- Zone -------------------------------------------------
 if ($LaneIP -ne $null -or $LaneIP -ne $null) 
	{	 
		     if ( $LaneIP.Substring(4,2) -eq "88") {$Zone="SKYWAY" } 
		 elseif ( $LaneIP.Substring(4,2) -eq "89") {$Zone="SLEX" } 
		 elseif ( $LaneIP.Substring(4,2) -eq "25") {$Zone="STAR" } 
		 elseif ( $LaneIP.Substring(4,2) -eq "21") {$Zone="NAIAX" } 
		 else {$Zone = "Unknown"}
	}
	else {$Zone = "Unknown"}
...
# -------------- Plaza ID -------------------------------------------------
 if ($LaneIP -ne $null -or $LaneIP -ne $null) 
	{	 
		$PlazaID = $LaneIP.Substring(7,3)
	}
	else {$PlazaID = "Unknown"}

# ----------- Add CSV File -----------
    $AddLine = $AutoDateTime2 + "," + $SelectFile.FullName + "," + $SelectFile + "," + 
                ...
                $ERR_NOVEHICLE.Count + "," + $ERR_SMDEBUG_PROBLEM.Count  + "," + 
				...
				$VPCVERSION + "," + $VPCRESETCOUNTER + "," + $CMD_GET_PROBS_ENABLE + "," + "$Debounce_$Debounce2" + "," + 
				...
				$SuccessEntryRequestRate + "," +
				$LaneIP + "," + $LogDesc + "," + $Zone + "," + $PlazaID
...
#							$LogDesc
    ...
    if (!(Test-Path $LogFileName)) {
               $Title = "AnalysisDateTime" + "," + "FullPath" + "," + "FileName" + "," + 
			   			"Average" + "," + "BeginNew" + "," + 
						...
						"SuccessRequestEntryRate" + "," + 
						"LaneIP" + "," + "Decription" + "," + "Zone" + "," + "PlazaID"
